def ETS(data, historyFromDate, historyToDate, selectedFromDate, selectedToDate):
    import numpy as np
    import pandas as pd
    ...
    for index, row in data.iterrows():
        y = row.values
        # Convert the string list to numeric values
        y_numeric = list(map(float, y))
        # Fit the model
        ...
        seasonal=None)
        model = model.fit()
        temp = len(get_months_between_dates(parsed_dates[1], parsed_dates[3]))-1
        temp2 = len(get_months_between_dates(parsed_dates[1], parsed_dates[2]))-2
        print(temp, temp2)
        forecast = list(model.forecast(steps=temp))[temp2:]
        print(len(forecast))
        forecast_res[index] = forecast
